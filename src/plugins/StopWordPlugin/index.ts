...
// const STOP_WORDS = ['для', "продолж|ать|ить|ила|ил", "продолж|ать|ить|ила|ил работу"]
const STOP_WORDS = ['для', "продолжать", "продолжить", "продолжила", "продолжил"]

const FULL_STOP_WORDS = STOP_WORDS.map(word => {
    if (word.indexOf("|") !== -1)
        return word
    word.split("|")
})
...
const getStopWord = () => {
    // return `(^|$|[${STOP_WORDS.map(str => `(${str})`)}])`;
    return STOP_WORDS.map(str => `(${str})`).join("|");
}

const REGEX = new RegExp(getStopWord(), 'i');

export function StopWordPlugin(): JSX.Element | null {
